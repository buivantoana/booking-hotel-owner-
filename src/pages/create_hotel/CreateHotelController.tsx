...
const submitCreateHotel = async (contextData: any) => {
  ...
  for (const room of roomTypes) {
    ...
    roomForm.append("name", JSON.stringify({ vi: room.name || "Phòng tiêu chuẩn" }));
    roomForm.append("description", JSON.stringify({ vi: room.description || "" }));
    ...
    // Giá tiền
    roomForm.append("price_hourly", room.pricing.hourly.enabled && room.pricing.hourly.firstHours ? room.pricing.hourly.firstHours : "0");
    roomForm.append("price_hourly_increment", room.pricing.hourly.enabled && room.pricing.hourly.extraHour ? room.pricing.hourly.extraHour : "0");
    roomForm.append("price_overnight", room.pricing.overnight.enabled && room.pricing.overnight.price ? room.pricing.overnight.price : "0");
    roomForm.append("price_daily", room.pricing.daily.enabled && room.pricing.daily.price ? room.pricing.daily.price : "0");
    ...
    roomForm.append("direction", JSON.stringify({ vi: room.direction || "" }));
  ...
  return (
    <CreateHotelView submitCreateHotel={submitCreateHotel} />
  )
}
